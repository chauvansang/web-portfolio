...
export default class Intro extends Component {
  ...
  animateNext = (index) => {
    ...
    if (index < refObject.length - 1) {
      ...
      if (index == 3 && refObject[3].ref.current && refObject[4].ref.current) {
        this.secondDivMargin =
          refObject[3].ref.current.offsetWidth / 2 -
          refObject[4].ref.current.offsetWidth / 2
      }
      setTimeout(
        () => this.animateNext(index + 1),
        refObject[index].nextTimeoutDuration
      )
    } else {
  ...
  updateDivPositions(referenceObjectIndex) {
    ...
    const {
      ref: { current },
      direction,
    } = refObject[referenceObjectIndex]
    ...
    if (direction == 'right') {
      ...
      const resultDivPositionY = Math.abs(
        divPositionY + componentHeightPadding + objectHeight / 2
      )
      refObject[referenceObjectIndex] = {
        ...refObject[referenceObjectIndex],
        isVisible: true,
      ...
      const resultDivPositionX = Math.abs(
        secondPreviousObjectWidth / 2 + previousObjectWidth / 2
      ) // TODO rethink
      const resultDivPositionY = Math.abs(
        divPositionY + componentHeightPadding + objectHeight / 2
      )
      ...
      <Div justify align style={style} className={styles.intro_container}>
        ...
        >
          ...
          <div className={styles.inline_container}>
            <Div row align>
              <div
                ...
                className={`${styles.intro_text} ${
                  styles.reveal_animate_skew
                } ${refObject[1].isVisible ? styles.animate : ''}`}
              >
                Welcome &nbsp;
              </div>
              <div className={styles.reveal_container}>
                <span
                  ref={refObject[2].ref}
                  className={`${styles.intro_text} ${
                    styles.reveal_animate_right
                  } ${refObject[2].isVisible ? styles.animate : ''}`}
                >
                  Just so you know
                </span>
              </div>
            ...
            <div
              ref={refObject[3].ref}
              className={`${styles.intro_text} ${styles.flip_animate_top} ${
                refObject[3].isVisible ? styles.animate : ''
              }`}
            >
          ...
          <Div row style={{ overflow: 'hidden' }}>
            <span
              ref={refObject[4].ref}
              className={`${styles.intro_text} ${styles.reveal_animate_top} ${
                refObject[4].isVisible ? styles.animate : ''
              }`}
            ...
            <Div align>
              <span
                ref={refObject[5].ref}
                className={`${styles.intro_text} ${styles.animate_right} ${
                  refObject[5].isVisible ? styles.animate : ''
                }`}
              >
              ...
              <span
                ref={refObject[6].ref}
                className={`${styles.intro_text} ${
                  styles.reveal_center_animate_skew
                } ${refObject[6].isVisible ? styles.animate : ''}`}
              >
                🙈
              </span>
            </Div>
        ...
        <div className={styles.cover_reveal_container}>
          <div
            className={`${styles.cover_reveal_layer} ${styles.animate_layer}`}
          ></div>
        </div>
      </Div>
    )
