...
import {
  getProjectImages,
  imageSpecificRatio,
} from 'Constants/projectImageConstants'
...
const ProjectImageGrid = ({ projectId, gridItemSelected }) => {
  ...
  const onImageLoad = (image, projectImage) => {
    ...
    if (loadedProjectImages.current.length == projectImages.length) {
      loadedProjectImages.current = loadedProjectImages.current.sort(
        (currentImage, nextImage) => {
          if (currentImage.index > nextImage.index) return 1
          else if (currentImage.index < nextImage.index) return -1
  ...
  const visibleProjectImages = showLoadedImage
    ? loadedProjectImages.current
    : projectImages
...
const getImageRatio = (projectImage) => {
  ...
  if (specificProjectImages) {
    const specificProjectImage = specificProjectImages.find(
