...
class Landing extends Component {
  ...
  handleOrientation = (event) => {
    this.setState({
      clientX: Math.floor(event.gamma) * 10,
      clientY: Math.floor(event.beta) * 10,
    })
  }
  ...
  getBodyContent = (bodyType, screenSize) => {
    ...
    return (props) => (
      <Div fillParent style={props} className={styles.body_content_container}>
        {bodyType == landingPageBody.PROJECT &&
          (showMobile ? (
            <ProjectsMobile updateBodyType={this.updateBodyType} />
          ) : (
            <Projects />
          ))}
        {bodyType == landingPageBody.TIMELINE &&
          (showMobile ? (
            <TimelineMobile updateBodyType={this.updateBodyType} />
          ) : (
            <Timeline />
          ))}
      </Div>
