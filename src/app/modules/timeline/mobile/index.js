...
class TimelineMobile extends Component {
  ...
  componentDidMount() {
    ...
    detectSwipe(this.containerRef.current, (direction) => {
      ...
      const index = timelineListValue.findIndex(
        (element) => element.id === selectedTimelineId
      )

      if (direction == 'l') {
        if (index < timelineListValue.length - 1) {
          this.onTimelineSelected({
            selectedId: timelineListValue[index + 1].id,
          })
        } else {
        ...
        if (index != 0) {
          ...
          this.onTimelineSelected({
            selectedId: timelineListValue[timelineListValue.length - 1].id,
          })
        }
      }
    })
  ...
  onTimelineSelected = ({ selectedId }) => {
    ...
    this.setState(
      { selectedTimelineId: selectedId, projectsList, currentSlide: 0 },
      () => {
      ...
      >
        ...
        <Div fillParent className={styles.content_container}>
          ...
          <Div fillParent className={styles.view_pager_container}>
            <Swiper {...params} getSwiper={this.onSwiperMount}>
              {map(projectsList, (project, index) => (
                <Div key={index} align justify>
                  <ProjectListItem
                    project={project}
                    className={`${
                      index == currentSlide
                        ? styles.project_list_item__selected
                        : ''
                    } ${styles.project_list_item}`}
                  />
                </Div>
              ))}
            </Swiper>
          ...
          <Div
            alignSelf='center'
            row
            className={styles.pagination_button_container}
          >
            <PaginationButton
